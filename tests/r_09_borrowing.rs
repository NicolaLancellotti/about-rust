...
fn lifetime() {
    ...
    }
    {
        ...
        let x = 10;
        let mut s: Structure<'_> = Structure { ref1: &x };

        {
            let x = 10;
            // s = Structure{ref1: &x}; // Error
        }
        assert_eq!(*s.ref1, 10);
    }
    {
...
fn trait_object_lifetimes() {
    ...
    */
    ...
    impl<'a> Trait for Structure1<'a> {}

    struct Value<'a> {
        value: &'a i32,
    }

    struct Structure2<'a, T: 'a> {
        x: &'a i32,
        y: T,
    }

    impl<'a, T> Trait for Structure2<'a, T> {}

    struct Structure3<'a, 'b: 'a, T: 'a, U: 'b> {
        x1: &'a i32,
        x2: &'b i32,
        y1: T,
        y2: U,
    ...
    impl<'a, 'b, T, U> Trait for Structure3<'a, 'b, T, U> {}
    ...
    fn trait_object_lifetime<'a: 'b, 'b: 'a, 'c>(
        ...
        let trait_object: Box<dyn Trait> = Box::new(Structure1 { x: &10 });
        let trait_object_explicit: Box<dyn Trait + 'static> = Box::new(Structure1 { x: &10 });

        let trait_object: Box<dyn Trait> = Box::new(Structure1 { x: &num });
        let trait_object_explicit: Box<dyn Trait + 'a> = Box::new(Structure1 { x: &num });

        let trait_object: Box<dyn Trait> = Box::new(Structure2 { x: &num, y: value1 });
        let trait_object_explicit: Box<dyn Trait + 'a> =
            Box::new(Structure2 { x: &num, y: value2 });

        let trait_object: Box<dyn Trait> = Box::new(Structure3 {
            x1: &num,
            x2: &num2,
            y1: value3,
            y2: value4,
        });
        let trait_object_explicit: Box<dyn Trait + 'a> = Box::new(Structure3 {
            x1: &num,
            x2: &num2,
            y1: value5,
            y2: value6,
